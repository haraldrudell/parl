...
func (hp *Https) listenerThread() {
	...
	if err := srv.Serve(tlsListener); err != nil { // blocking until Shutdown or Close
		if err != http.ErrServerClosed {
			hp.SendErr(error116.Errorf("srv.ServeTLS: '%w'", err))
			return
		}
	}
}
...
func (hp *Https) TLS() (tlsListener net.Listener, err error) {
	...
	if err = srv.ServeTLS(nil, badPath, badPath); err == nil {
		err = error116.New("missing error from srv.ServeTLS")
		return
	} else {
		...
		if err != nil {
			err = error116.Errorf("srv.ServeTLS: '%w'", err)
			return
		}
	}
	...
	if !parls.StrSliceContains(tlsConfig.NextProtos, http11) {
