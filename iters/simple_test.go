...
func TestSimple(t *testing.T) {
	...
	var reset = func() {
		iterator = NewSimpleConverterIterator(NewSliceIterator(keys), newhasSimpleConverter(m).SimpleConverter)
	}
...
type hasSimpleConverter[K comparable, V any] struct{ m map[K]V }

func newhasSimpleConverter[K comparable, V any](m map[K]V) (s *hasSimpleConverter[K, V]) {
	return &hasSimpleConverter[K, V]{m: m}
}
func (s *hasSimpleConverter[K, V]) SimpleConverter(key K) (value V) { return s.m[key] }
