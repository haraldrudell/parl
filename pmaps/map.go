...
func NewMap[K comparable, V any]() (mapping *Map[K, V]) { return &Map[K, V]{m: make(map[K]V)} }
...
func (m *Map[K, V]) Put(key K, value V) { m.m[key] = value }

// Delete removes mapping for key
//   - if key is not mapped, the map is unchanged.
//   - O(log n)
func (m *Map[K, V]) Delete(key K) { delete(m.m, key) }

// Length returns the number of mappings
func (m *Map[K, V]) Length() (length int) { return len(m.m) }
